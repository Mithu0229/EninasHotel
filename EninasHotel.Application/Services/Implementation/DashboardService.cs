...
namespace EninasHotel.Application.Services.Implementation
{
    ...
    public class DashboardACMService : IDashboardACMService
    {
        private readonly IUnitOfWork _unitOfWork;
        static int previousMonth = DateTime.Now.Month == 1 ? 12 : DateTime.Now.Month - 1;
        readonly DateTime previousMonthStartDate = new(DateTime.Now.Year, previousMonth, 1);
        readonly DateTime currentMonthStartDate = new(DateTime.Now.Year, DateTime.Now.Month, 1);
        public DashboardACMService(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }
        public async Task<PieChartDto> GetBookingPieChartData()
        ...
        public async Task<RadialBarChartDto> GetCashOutChartData()
        {

            var totalUsers = _unitOfWork.Amount.GetAll();

            var countByCurrentMonth = totalUsers.Count(u => u.TransactionDate >= currentMonthStartDate &&
            u.TransactionDate <= DateTime.Now);

            var countByPreviousMonth = totalUsers.Count(u => u.TransactionDate >= previousMonthStartDate &&
            u.TransactionDate <= currentMonthStartDate);
        ...
        public async Task<RadialBarChartDto> GetTotalCashInRadialChartData()
        {
            var totalBookings = _unitOfWork.Amount.GetAll(u => u.IsActive == true);

            var countByCurrentMonth = totalBookings.Count(u => u.TransactionDate >= currentMonthStartDate &&
            u.TransactionDate <= DateTime.Now);

            var countByPreviousMonth = totalBookings.Count(u => u.TransactionDate >= previousMonthStartDate &&
            u.TransactionDate <= currentMonthStartDate);
